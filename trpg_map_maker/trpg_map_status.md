...
- ❌ **ユーザー画像アップロード (装飾/パターン)** — png/jpg/svg/webp を装飾またはパターンとして利用可能に
    - **保存先**: IndexedDB (既存マップ DB に object store を1つ追加。`{ id, name, blob, type: 'decor'|'pattern', meta }`)
    - **スコープ**: マップごと (シナリオ毎の独立性を優先。グローバル共有は後で足せる)
    - **対応形式**: png / jpg / svg / webp。シームレスでない画像のエッジ処理は特に修正せずユーザーに委ねる
    - **保存形式**: アップ時に `<canvas>` で webp 化してから保存 (容量削減)
    - **UI 配置**: 装飾とパターンの各パネル末尾に「+ 画像をアップロード」ボタン、各サムネ右上に削除ボタン
    - **既存実装との統合**: ハードコード `DECORS[]` / `PATTERNS[]` とは別に `App.userDecors[]` / `App.userPatterns[]` を持ち、`getDecorDef` / `getPatternDef` で統合参照
- ❌ **オブジェクトのエッジぼかし** — 霧・幽霊・夢など「ふわっとした塗り」エフェクト
    - 方針: **blur.html 方式** (`fabric.Image.filters.Blur` でラスタライズ後にぼかし → `destination-in` でマスク)
    - 元シェイプは非表示で保持し、編集時はシェイプを見せ、確定時に blur 版を再生成・差し替え
    - Undo スナップショットは元シェイプ + blurパラメータのみ保存 (巨大 dataURL を履歴に乗せない)
    - **ユーザー画像アップロード機能の後に実装** (画像扱いの基盤を共用するため)
- ❌ **道ツール (road / railway / river)** — 折線・曲線で細長いものを描画する専用ツール
    - 設計方針: 等方テクスチャ (砂利・水面・アスファルトノイズ) は SVG ストローク + パターン、方向性テクスチャ (線路の枕木・車線・木目板) はパス沿いタイル配置のハイブリッド
    - パターンを SVG パスの接線方向に追従させることは Canvas/SVG では不可能なため、用途別に手段を分ける
    - タイル方式は `テスト/bbbb.html` にプロトタイプあり (M/Q/T パース → 分割 → 接線回転で画像配置)。本番では弧長ベースの等間隔配置に拡張する必要あり
    - 構造案: 道タイプ (道路 / 線路 / 川 / 小道 / 木橋) のプリセット、本体ストローク + 装飾タイルレイヤーをグループ化
